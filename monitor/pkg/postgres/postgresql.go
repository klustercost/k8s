...
func NewPostgresql(user, password, dbname, dbhost, dbport string) (*Postgresql, error) {
	p := &Postgresql{}
	connectionString := fmt.Sprintf("user=%s password=%s dbname=%s host=%s port=%s sslmode=disable", user, password, dbname, dbhost, dbport)
	var err error
...
func (p *Postgresql) InsertOwner(name string, namespace string, record_time time.Time, own_version, own_kind, own_uid, owner_version, owner_kind, owner_name, owner_uid, labels string) error {
